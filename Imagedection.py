...
while True:
    ...
    blurred = cv2.GaussianBlur(frame,(11,11),0)#smoooting
    hsv  = cv2.cvtColor(blurred,cv2.COLOR_BGR2HSV)#convert bgr to hsv color formate
    ...
    if len(cnts)>0:
            ...
            M = cv2.moments(c)
            center = (int(M["m10"]/M["m10"]),int(M["m10"]/M["m10"]))
            if radius>10:
